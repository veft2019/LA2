...
const auctionService = () => {
    ...
    const getAuctionWinner = async (auctionId) => {
        return await globalTryCatch(async () => {
            ...
            const highestBid = await dbProvider.AuctionBid.find({ auctionId: auctionId }).sort({ price: -1 });
	...
	const getAuctionBidsWithinAuction = async (auctionId) => {
        return await globalTryCatch(async () => {
            ...
            const bids = await dbProvider.AuctionBid.find({ auctionId: auctionId }).sort({ price: -1 });
            return {
                status: 200,
                body: bids
            }
	...
	const placeNewBid = async (auctionId, customerId, price) => {
        ...
        await dbProvider.Auction.findById(auctionId).updateOne({ auctionWinner: customerId });
        //console.log(stuff);
